"""
Healthy-set calibration — pixel-space (no VAE / no latent fusion).
===================================================================
Runs DDIM partial-noise reconstruction on held-out HEALTHY slices to
establish, per timestep level T:

  * ``M_baseline``  — mean residual_stack over healthy tissue   (K, 256, 256)
  * ``pixel_scale`` — healthy normalisation factor
...
# ═════════════════════════════════════════════
@torch.no_grad()
def calibrate_single_t(unet, val_loader, device, *, t_int=C.T_INT,
                       ddim_steps=C.DDIM_STEPS, max_samples=C.MAX_CAL_SAMPLES,
                       percentile=C.THRESHOLD_PERCENTILE, generator=None):
...
# ═════════════════════════════════════════════
@torch.no_grad()
def calibrate_multi_t(unet, val_loader, device, *, t_list=None,
                      ddim_steps=C.DDIM_STEPS, max_samples=C.MAX_CAL_SAMPLES,
                      percentile=C.THRESHOLD_PERCENTILE,
                      agg_mode=C.MULTI_T_AGG, generator=None):
    ...
    for images in val_loader:
        ...
        for t in t_list:
            ...
            orig, recon = reconstruct_healthy(unet, ddim, images, ts, t, generator)
            o, r = orig.cpu().numpy(), recon.cpu().numpy()
            for b in range(bs):
                st = residual_stack(o[b], r[b])
                resid_sum[t] = st.copy() if resid_sum[t] is None else resid_sum[t] + st
                cache[t].append((st, brain_mask_2d(o[b])))
        n += bs
    if n == 0:
        raise RuntimeError("Calibration set is empty.")
    ...
    calib = {"multi_t": True, "t_list": t_list, "ddim_steps": int(ddim_steps),
             ...
             "per_t": {str(t): {"pixel_scale": scales[t]} for t in t_list}}
...
# ─────────────────────────────────────────────
@torch.no_grad()
def score_image_multi_t(unet, ddim, image, baselines, calib, generator=None):
    ...
    for i, t in enumerate(t_list):
        ts = inference_timesteps(ddim, t, ddim_steps)
        orig, recon = reconstruct_healthy(unet, ddim, image, ts, t, generator)
