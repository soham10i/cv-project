...
amplification that is particularly useful in the low-patient pediatric regime --- and the
...
\section{Counterfactual generation: faithfulness and prototype mapping}
...
map $|\x-\x'|$ is the counterfactual difference (Wachter et al., 2017; Sanchez et
al., 2022; Wolleb et al., 2022).

\paragraph{Faithfulness.} An explanation $\mathcal E(\x)$ is \emph{faithful} if it
is a function of the actual decision computation, not of a separate surrogate. Here
the binary decision is $\hat P(\bm u)=\mathbb 1[A(\bm u)>\tau]$ with
$A$ built directly from $|\x-\x'|$ (Eqs.~\eqref{eq:residual}--\eqref{eq:multiscale});
the explanation $|\x-\x'|$
is therefore \emph{the decision variable itself}, so faithfulness holds by
construction --- there is no approximation gap.

\paragraph{Prototype / manifold mapping.} Let $\mathcal M_h$ be the support of the
learned healthy distribution. Partial-noise DDIM (\S\ref{sec:partial}) sends $\x$
to its denoised image $\x'=\hat{\x}_0^{\text{healthy}}$, which approximates the
\emph{projection} of $\x$ onto $\mathcal M_h$ under the metric induced by the
learned score,
\begin{equation}
\x'\;\approx\;\Pi_{\mathcal M_h}(\x)
\;=\;\argmin_{\bm m\,\in\,\mathcal M_h}\; d_\theta(\x,\bm m),
\label{eq:prototype}
\end{equation}
i.e.\ the nearest \emph{healthy prototype} of the patient. The counterfactual
difference $|\x-\x'|$ is thus the minimal, on-manifold edit that removes the
anomaly, and the iterates of the DDIM update~\eqref{eq:ddim} form the
\emph{healing trajectory} that visualises this projection step by step.

\section{Why post-hoc class-activation maps are unfaithful here}
Class-activation methods such as Grad-CAM (Selvaraju et al., 2017) localise the
regions that raise a single \emph{class logit}. A diffusion denoiser has no class
logit; the only available scalar is the dense-regression noise error
$y=\sum_{\bm u}\|\eps_\theta(\x_t,t)-\bm n\|^2$, whose global-average-pooled
gradient $\alpha_k=\frac{1}{HW}\sum_{ij}\partial y/\partial A^k_{ij}$ collapses all
spatial information into one scalar per channel, leaving
$\ReLU(\sum_k\alpha_k\bm A^k)$ with no spatial selectivity. Empirically the map is
diffuse \emph{even on slices where the anomaly score sharply localises the lesion},
i.e.\ it fails the sanity criterion that an explanation should track the model's
actual output (Adebayo et al., 2018) and violates the faithfulness requirement
(Rudin, 2019). We therefore do \emph{not} use Grad-CAM as an explainer; we retain
it only as a documented negative result (\texttt{src/xai/gradcam.py}) and rely on
the faithful counterfactual and the exact residual attribution below.
...
(\texttt{src/xai/attribution.py}). In future work, attention-rollout maps (Abnar
\& Zuidema, 2020) --- which trace the UNet's self-attention weights at the
12$\times$12 bottleneck back to input spatial positions --- could provide a
model-internal view to complement the external counterfactual explanation. The
config flag \texttt{attention\_rollout} is wired; the visualisation pipeline has
...
\paragraph{Counterfactual sanity.} A trustworthy counterfactual concentrates the
difference $|\x-\x'|$ on the lesion and co-localises with both the anomaly score
$S$ and the ground truth $G$; a diffuse counterfactual (high residual across the
whole parenchyma) signals an out-of-manifold reconstruction rather than localised
pathology --- an intrinsic, label-free check that flags the failure cases.
...
Eq.~\eqref{eq:prototype} (counterfactual): \texttt{xai/counterfactual.py},
\texttt{xai/attribution.py}.
...
\item Behrendt, Bhattacharya, Kr\"uger, Opfer, Schlaefer (2023). \textit{Patched Diffusion Models for UAD in Brain MRI}. MIDL.
...
\item Rudin (2019). \textit{Stop Explaining Black Box Models for High-Stakes Decisions and Use Interpretable Models Instead}. Nature Machine Intelligence.
\item Wachter, Mittelstadt, Russell (2017). \textit{Counterfactual Explanations without Opening the Black Box}. Harvard JOLT.
\item Wolleb, Bieder, Sandk\"uhler, Cattin (2022). \textit{Diffusion Models for Medical Anomaly Detection}. MICCAI.
