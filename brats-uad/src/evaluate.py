"""
Stage 4 — Evaluate anomaly detection on test lesion slices.
============================================================
...
it, and reports per-image + aggregate AUROC / AUPRC / DICE.  Saves qualitative
panels (original / healthy-recon / anomaly map / prediction / GT) for the report.
...
from pipeline.diffusion import (make_ddim_scheduler, inference_timesteps,
                                encode_to_latents, reconstruct_healthy)
from pipeline.scoring import (brain_mask_2d, pixel_residual_2d, latent_residual_2d,
...
def evaluate(args):
    ...
    use_fusion = (mcalib if use_multi else calib)["use_fusion"]
    log.info("Mode: %s | threshold %.4f | fusion=%s",
    ...
    for i in range(len(ds)):
        ...
        if use_multi:
            score, bmask, recon = score_image_multi_t(
                vae, unet, ddim, image, baselines, mcalib, gen, sf)
...
if __name__ == "__main__":
    try:
        p = argparse.ArgumentParser(description="Stage 4 — anomaly detection evaluation")
