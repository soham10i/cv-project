"""
Stage 2 — Train the healthy-manifold latent DDPM.
==================================================
Trains a UNet2DModel to denoise latents of the frozen medical KL-VAE, using ONLY
lesion-free, buffer-clean slices from TRAIN patients.  At inference, partially
noising a test slice and denoising with this model projects it back onto the
healthy manifold; the residual reveals lesions.

Unlike the old pipeline (≤50 slices), this trains on the full healthy slice pool
(thousands of slices) so the manifold is actually learned.

Usage
-----
    python src/train_diffusion.py                  # defaults
    python src/train_diffusion.py --epochs 40 --bs 32
...
from pipeline.diffusion import (make_ddpm_scheduler, encode_to_latents)
...
def train_epoch(unet, vae, sched, loader, opt, ema, device, sf, epoch, total, grad_accum):
    unet.train()
    running, n = 0.0, 0
    nb = len(loader)
    opt.zero_grad()
    for bidx, images in enumerate(loader):
        ...
        latents = encode_to_latents(vae, images, sample=True, scaling_factor=sf)
...
@torch.no_grad()
def validate(unet, vae, sched, loader, device, sf, max_batches=25):
    unet.eval()
    ...
    for bi, images in enumerate(loader):
        if bi >= max_batches:
            break
        images = images.to(device)
        latents = encode_to_latents(vae, images, sample=False, scaling_factor=sf)
        bsz = latents.shape[0]
...
def main(args):
    ...
    rl = RunLogger("diffusion", config_snapshot=C.snapshot(),
                   run_name=("diffusion_smoke" if args.smoke else None),
    ...
    print("=> Loading VAE from local disk...")
    vae = KLVAE.from_pretrained(C.VAE_DIR, map_location=device).to(device).eval()
    vae.requires_grad_(False)
    print("=> VAE loaded.")
    
    print("=> Loading scaling factor...")
    sf = C.load_scaling_factor()
    print(f"=> Scaling factor loaded: {sf}")
    log.info("Frozen VAE loaded | scaling_factor=%.5f", sf)

    print("=> Building UNet and Scheduler...")
    unet = build_unet(device)
    sched = make_ddpm_scheduler()
    print("=> UNet and Scheduler built.")
    ...
    log.info("=" * 60)
    log.info("Diffusion train | %d ep | bs %d×accum %d | lr %.1e | wd %.0e | EMA %.4f | "
             "patience %d%s", epochs, bs, args.grad_accum, args.lr, args.weight_decay,
    ...
    for epoch in range(start_epoch, epochs + 1):
        t0 = time.time()
        avg = train_epoch(unet, vae, sched, train_loader, opt, ema, device, sf,
                          epoch, epochs, args.grad_accum)
        lr_sched.step()
        vloss = validate(unet, vae, sched, val_loader, device, sf, args.val_batches)
        history.append(avg)
...
if __name__ == "__main__":
    try:
        p = argparse.ArgumentParser(description="Stage 2 — latent DDPM on healthy slices")
